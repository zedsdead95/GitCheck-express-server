...
class Github {
  ...
  licenceData(repoName) {
    return this.request(`/repos/${repoName}/contents/license`);
  }

  repoData(username, repoName) {
    return this.request(`/repos/${username}/${repoName}/contents`);
    // TODO loop on the JSON to find what we need using wildcards, i.e *eslint*, *test* folder etc
  }
}
